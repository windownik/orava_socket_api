...
async def handler_change_msg(msg: dict, db: Depends, websocket: WebSocket, manager: ConnectionManager,
                             reqwest_user: User):
    change_msg = ChangeMsg.parse_obj(msg)
    socket_resp = SocketRespGetUpdates()

    # Проверяем права доступа на сообщение
    owner_id = await conn.get_token(db=db, token_type='refresh', token=change_msg.refresh_token)
    if not owner_id:
    ...
    print(change_msg.message.msg_id, msg_data)
