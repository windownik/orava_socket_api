...
async def save_msg(db: Depends, msg: dict):
    now = datetime.datetime.now()
    data = await db.fetch(f"INSERT INTO messages (text, from_id, reply_id, chat_id, to_id, status, create_date) "
                          f"VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT DO NOTHING RETURNING *;",
                          msg['text'], msg['from_id'], msg['reply_id'], msg['chat_id'], msg['to_id'], msg['status'],
